...
import { TestChaoswapV1SwapPay } from '../typechain/TestChaoswapV1SwapPay'
...
import { TestChaoswapV1ReentrantCallee } from '../typechain/TestChaoswapV1ReentrantCallee'
...
describe('ChaoswapV1Pool', () => {
  ...
  let factory: ChaoswapV1Factory
  let pool: MockTimeChaoswapV1Pool

  let swapTarget: TestChaoswapV1Callee
  ...
  async function initializeAtZeroTick(pool: MockTimeChaoswapV1Pool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  // https://github.com/Chaoswap/Chaoswap-v1-core/issues/214
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestChaoswapV1ReentrantCallee')
      ).deploy()) as TestChaoswapV1ReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestChaoswapV1SwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestChaoswapV1SwapPay')
      underpay = (await underpayFactory.deploy()) as TestChaoswapV1SwapPay
