...
contract TestChaoswapV1Router is IChaoswapV1SwapCallback {
    ...
    function swapForExact0Multi(
        ...
        pools[0] = poolInput;
        IChaoswapV1Pool(poolOutput).swap(
            recipient,
            false,
            -amount0Out.toInt256(),
    ...
    function ChaoswapV1SwapCallback(
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid =
                amount0Delta > 0 ? IChaoswapV1Pool(msg.sender).token0() : IChaoswapV1Pool(msg.sender).token1();
            int256 amountToBePaid = amount0Delta > 0 ? amount0Delta : amount1Delta;

            bool zeroForOne = tokenToBePaid == IChaoswapV1Pool(pools[0]).token1();
            IChaoswapV1Pool(pools[0]).swap(
                msg.sender,
                zeroForOne,
                -amountToBePaid,
            ...
            if (amount0Delta > 0) {
                IERC20Minimal(IChaoswapV1Pool(msg.sender).token0()).transferFrom(
                    payer,
                    msg.sender,
                    uint256(amount0Delta)
                );
            } else {
                IERC20Minimal(IChaoswapV1Pool(msg.sender).token1()).transferFrom(
                    payer,
                    msg.sender,
